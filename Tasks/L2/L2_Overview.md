...
| T27 | EffectWindow 时间窗口框架 | P0 前置 | M | T25 | ✅ 已实现（1167 测试） |
| T28A | 新增触发器（7 个） | P1 核心 | S | T25 | ✅ 已实现（1167 测试） |
| T28B | 新增即时原子（4 个） | P1 核心 | M | T27 | ✅ 已实现（1167 测试） |
| T29 | SnakePartsManager + 蛇头链 | P1 核心 | L | T27, T28 | ✅ 已实现（1209 测试） |
| T30 | 蛇尾链（Salamander + Lag Tail） | P1 核心 | M | T27, T28 | ✅ 已实现（1266 测试） |
| T31 | ScaleSystem 槽位管理 + 9 鳞片 | P1 核心 | L | T28 | ✅ 已实现（1332 测试） |
| T32 | 邻接共鸣系统 | P2 增强 | M | T31 | ✅ 已实现（1429 测试） |
| T33 | 全系统联调 + Build 测试面板 | P2 增强 | L | T29-T32 | ✅ 已实现（1518 测试） |
