...
- **StatusCarrier 统一载体 + ReactionResolver 反应引擎（T27A）** — 蛇段/敌人/状态格统一实现 StatusCarrier 接口
  - 每种状态类型在载体上最多一个，无叠层，支持多状态共存
  - CollisionHandler 统一处理碰撞（替代 5 处硬编码逻辑）
  - ReactionResolver JSON 驱动反应规则（替代 3 处重复 `_get_reaction_id`）
  - Policy 机制：immediate（立即反应）/ stable（延迟到指定事件）
