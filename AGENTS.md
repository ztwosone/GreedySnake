...
| Agent 控制面 | `AgentOps/` | 会话无关的任务派发、验证、交接与当前状态 |
...
- **体验深度、认知轻度**：新增机制必须能和既有蛇身状态、敌人、Build、共鸣或房间流程产生组合关系；但单个任务不得一次引入多个玩家必须理解的新概念。
- **渐进暴露**：新规则先在低压场景出现，再进入混合战斗或高压房间。
- **可读反馈**：玩家必须能从颜色、位置、状态图标、文本标签或简单 UI 看懂当前发生了什么。
- **可回退**：复杂机制必须能通过 JSON 配置禁用、降级或调参，方便调试和平衡。

## 美术与 UI 占位政策

- 美术/UI 不阻塞玩法验证；L3 v1 可以使用色块、文本标签、简单图标和调试面板。
- 新系统必须预留表现接口和节点/数据入口，后续可替换为正式资产。
- L3 gameplay 验收只检查信息是否清楚、状态是否可辨、交互是否可完成，不检查最终美术品质。
...
L3+ agent 接管默认从 `AgentOps/README.md` 启动，由 Orchestrator Agent 按单任务卡派发，不依赖聊天上下文。
